...
class Graph(CreateData):
    ...
    def create_graph(self, pur_or_pro):
        ...
        dt = plt.scatter(self.date, pur_or_pro, c=self.second_color, marker='o')
        ...
        def on_add(sel):
            xi, yi = sel.target
            xi = int(round(xi))
            sel.annotation.set_text(f'Дата: {self.date[xi]}\nЗначение: {yi}')
    ...
    def create_graph_bar(self, mode, overall):
        ...
        def on_add(sel):
            ...
            sel.annotation.set(text=f'Дата: {self.date[sel.index][-2:]} число\nЗначение: {height}', position=(0, 20), anncoords="offset points")
